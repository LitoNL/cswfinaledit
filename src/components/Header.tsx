...
const Header: React.FC = () => {
  ...
  const handleContactClick = () => {
    setIsMenuOpen(false);
    // Ensure we scroll to top when navigating to contact
    setTimeout(() => {
      window.scrollTo(0, 0);
    }, 100);
  };
    ...
    >
      <div className="container mx-auto px-4 flex justify-between items-center">
        <Link to="/" onClick={scrollToTop} className="flex items-center">
          <span className="font-heading text-xl sm:text-2xl md:text-3xl tracking-wider">
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-dark-100/95 backdrop-blur-md py-4">
          <div className="container mx-auto px-4 flex flex-col space-y-4">
            <button
              onClick={() => handleNavigation('how-it-works')}
              className="text-gray-300 hover:text-electric transition py-2 text-left"
            >
              How It Works
            </button>
            <Link
